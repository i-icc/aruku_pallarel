...
- 提案文のトーン/具体性は `spec/ai.md` を正とする
- 履歴一覧はホームからハーフモーダルで開き、履歴詳細は散歩中画面のレイアウトに揃える（`spec/ux.md` 準拠）
- 履歴詳細の下中央ボタンは「ホームに戻る」（`spec/ux.md` 準拠）
- 散歩開始/終了時はロゴが回転するトランジションローディングを挟む（`spec/ux.md` 準拠）
