...
## ホーム画面
- 現在地はホーム表示中も更新され、マップの中心に反映される
- 初期ズームは散歩中より少し近めに設定する
...
- THEN:
  ...
  - 開始処理中はロゴが回転するトランジションローディングを表示する
...
- THEN:
  - チャットはハーフモーダルで表示され、背面にマップが維持される
  - チャット画面で提案メッセージを閲覧できる
  - メッセージをタップすると該当する街灯ピン位置にフォーカスし、ピン選択状態になる
  - ピンをタップするとピンの直上にメッセージ内容のポップアップを表示する
  - ポップアップはマップ上の他の場所をタップすると閉じる
