...
- THEN:
  ...
  - bearer token の取得に失敗した場合はログイン画面へ戻る
...
- THEN:
  - Backend へ位置更新を送信し、Firestore に位置情報が追記される
...
- GIVEN:
  - 散歩開始済み
- WHEN: 位置更新が Backend に送信される
- THEN:
  - Backend が条件（前回 `ok` から 5 分 / 250m 以上の移動）を満たせば `ok` を返し非同期で提案生成
  - 条件未達の場合は `ng` を返す
